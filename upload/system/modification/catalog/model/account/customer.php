...
class ModelAccountCustomer extends Model {
	public function addCustomer($data) {
                ...
                $status = '1'; // переменная статуса блокировки 1-включено, 2-отключено (т.е. заблокирован)
				// сверяем IP or email с таблицей заблокированных 
				if( $this->isBanIpOrEmail($this->request->server['REMOTE_ADDR']) ) {
					$status = '0';
				}
				if( $this->isBanIpOrEmail($data['email']) ){
					$status = '0';
				}	
    ...
    public function getReferal($customer_id) {
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "customer_referral` WHERE customer_id = '" . $customer_id . "'");
        if(empty($query->row))
        {
            $str = $this->random_string(20,'upper');
            
            $this->db->query("INSERT INTO " . DB_PREFIX . "customer_referral SET customer_id = '" . $customer_id . "', ref1 = '" . $str . "'");
            
            return $str;
        }
        return $query->row['ref1'];
    }
    ...
    public function addReferal($referal,$customer_id) 
    {
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "customer_referral` WHERE ref1 = '" . $referal . "'");
        
        if(isset($query->row['customer_id']))
        {   
            $id1 =  $query->row['customer_id'];
            $this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "customer_customer SET customer_id = '" . $id1 . "', customer_ref = '" . $customer_id . "'");
        }
        
    }
    
    
    public function getRefSkidka($customer_id) 
    {
        $otp_query = $this->db->query("SELECT * FROM `oc_customer_discount` a  WHERE a.customer_id =  '" . $customer_id . "'");  
        
        return $otp_query->row;
    } 
    public function getBonusCust($customer_id) 
    {
        $otp_query = $this->db->query("SELECT * FROM `oc_customer_bonus` a  WHERE a.customer_id =  '" . $customer_id . "'");  
        if (empty($otp_query->row['bonus'])) return 0;
        return $otp_query->row['bonus'];
    } 
    
    public function setRefSkidka($customer_id,$discount) 
    {
        $sql = "UPDATE  `oc_customer_discount` SET  `discount` = '".$discount."' where customer_id = '".$customer_id."'" ;
        $this->db->query($sql);                     
                ...
                /***
                * Функция возвращает вхождение в таблицу заблокированного списка 
                */
                public function isBanIpOrEmail($ipOrEmail)
				{
					$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "customer_ban_ip` WHERE ip = '" . $this->db->escape($ipOrEmail) . "'");
					return $query->num_rows;
				}	
